...
class HRLAgent:
    ...
    def _select_low_action_with_blacklist(
        ...
        Args:
            state: 环境状态
            action_mask: 动作掩码 [n_actions]
            blacklist_info: 黑名单信息（保留参数但不使用）

        Returns:
            选择的动作（如果mask全0则返回-1表示失败）
        """
        # 1. 获取图嵌入
        graph_emb = self._get_graph_embedding(state)
        ...
        if action_mask is None:
            logger.warning("⚠️ 未提供动作掩码，使用全1掩码")
            mask_tensor = None
        else:
            # 检查有效动作
            valid_actions = np.where(action_mask > 0)[0]
            ...
            # ✅ 严格模式：mask全0直接返回失败
            if len(valid_actions) == 0:
                logger.error("❌ 所有动作被Mask！Episode应该失败")
                return -1  # 返回-1表示无效动作
        ...
        try:
            ...
            logger.error(f"❌ Low-Level选择失败: {e}")
        ...
        # 5. ✅ 最后安全校验（防止策略网络bug）
        if action_mask is not None:
            if action_idx < 0 or action_idx >= len(action_mask):
                logger.error(f"❌ 动作索引{action_idx}越界！")
                return -1

            if action_mask[action_idx] == 0:
                logger.warning(f"⚠️ 策略网络选择了被Mask的动作{action_idx}")
                valid_actions = np.where(action_mask > 0)[0]
                if len(valid_actions) == 0:
                    return -1
                action_idx = valid_actions[0]
                logger.debug(f"→ 修正为第一个有效动作: {action_idx}")
