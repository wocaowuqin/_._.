...
class Phase1ExpertCollector:
    ...
    def _convert_request_indices(self, raw_req):
        """
        🔥 [新增] 转换请求索引：1-based → 0-based
        模仿 sfc_env.py 的 reset_request 逻辑
        """
                ...
                if expert_result is not None:
                    ...
                    if tree_data is not None:
                        ...
                        if self.env.resource_mgr.apply_tree_deployment(deployment_plan, req):
                            ...
                            paths_to_save = []
                            for dest, path_nodes in paths_map.items():
                                # 🔥 转换为0-based并确保都是int类型
                                path_0 = [int(n - 1 if n > 0 else 0) for n in path_nodes]
                                paths_to_save.append(path_0)

                            # 🔥 req已经在上面转换过了，直接用即可
                            # 不需要再调用_sanitize_request，因为req已经是字典
                            clean_req = req.copy()  # 保存转换后的请求

                            for path in paths_to_save:
                                self.success_samples.append({
                                    "state": state_to_save,
                                    "request": clean_req,  # 已转为0-based的字典
                                    "action": {"path": path},  # 已是纯int列表
                                    "cost": 0.0,
                                    "load": self._estimate_load(),
                                    "hrl_info": {
                                        "subgoal": int(path[-1]),  # 🔥 确保是int
                                        "full_path": path
                                    }
                                })
